...
const wutchVideoSchema = z.object({
...
type WutchVideoFormData = z.infer<typeof wutchVideoSchema>;

interface EditWutchVideoDialogProps {
  video: {
    id: string;
    title: string;
...
export function EditWutchVideoDialog({ video, open, onOpenChange, onUpdate }: EditWutchVideoDialogProps) {
  const { user } = useAuth();
  const [isLoading, setIsLoading] = useState(false);
  const [thumbnailFile, setThumbnailFile] = useState<File | null>(null);
  const [thumbnailPreview, setThumbnailPreview] = useState<string>(video.thumbnail_url || "");

  const form = useForm<WutchVideoFormData>({
    resolver: zodResolver(wutchVideoSchema),
    defaultValues: {
      title: video.title,
  ...
  const onSubmit = async (data: WutchVideoFormData) => {
    ...
    try {
      ...
      console.error("Error updating wutch video:", error);
      toast({
        title: "Error",
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  className="hidden"
                  id="wutch-thumbnail-upload"
                />
                <Button
                  type="button"
                  variant="outline"
                  className="w-full"
                  onClick={() => document.getElementById("wutch-thumbnail-upload")?.click()}
                >
