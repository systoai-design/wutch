...
import { Upload, X, ExternalLink, Lock } from "lucide-react";
...
type ShortVideoFormData = z.infer<typeof shortVideoSchema>;

interface EditShortVideoDialogProps {
  video: {
    id: string;
    title: string;
...
export function EditShortVideoDialog({ video, open, onOpenChange, onUpdate }: EditShortVideoDialogProps) {
  const { user } = useAuth();
  const [isLoading, setIsLoading] = useState(false);
  const [thumbnailFile, setThumbnailFile] = useState<File | null>(null);
  const [thumbnailPreview, setThumbnailPreview] = useState<string>(video.thumbnail_url || "");

  const form = useForm<ShortVideoFormData>({
    resolver: zodResolver(shortVideoSchema),
    defaultValues: {
      title: video.title,
      description: video.description || "",
      promotional_link: video.promotional_link || "",
      promotional_link_text: video.promotional_link_text || "",
      is_premium: video.is_premium || false,
      x402_price: video.x402_price || 0.001,
    },
  });
  ...
  const onSubmit = async (data: ShortVideoFormData) => {
    ...
    try {
      ...
      if (thumbnailFile && user) {
        ...
        const fileName = `${user.id}/${video.id}-${Date.now()}.${fileExt}`;
      ...
      toast({
        title: "Error",
        description: "Failed to update short video",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Edit Short Video</DialogTitle>
          <DialogDescription>
            Update your short video details
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  className="hidden"
                  id="short-thumbnail-upload"
                />
                <Button
                  type="button"
                  variant="outline"
                  className="w-full"
                  onClick={() => document.getElementById("short-thumbnail-upload")?.click()}
                >
              ...
              <p className="text-xs text-muted-foreground">
                Recommended: 9:16 aspect ratio, max 5MB
              </p>
            </div>
